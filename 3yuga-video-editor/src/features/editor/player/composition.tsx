...
import { useCurrentFrame, Sequence } from "remotion"; // Import Sequence
...
const Composition = () => {
  ...
  const {
    ...
    structure,
    activeIds,
    duration // Import duration from useStore
  } = useStore();
  console.log("Composition duration from useStore:", duration);
  ...
  return (
    <>
      <Sequence durationInFrames={Math.round((duration / 1000) * fps)}>
        {Object.values(trackItemsMap).map((item) => {
          // Exclude transitions for now, they are handled by TransitionSeries
          if (item.type === "transition") return null;
          ...
          return SequenceItem[item.type](item, {
            ...
            size,
            isTransition: false,
            zIndex: item.metadata?.zIndex || 1 // Use item's zIndex or default to 1
          });
        })}
      </Sequence>
      {groupedItems.map((group, index) => {
        // Only render TransitionSeries for groups with actual transitions
        if (group.length > 1 && group.some(item => item.type === "transition")) {
            ...
            <TransitionSeries from={from} key={index}>
              {group.map((item) => {
                ...
                return SequenceItem[item.type](trackItemsMap[item.id], {
                  ...
                  isTransition: true,
                  size,
                  zIndex: trackItemsMap[item.id].metadata?.zIndex || 1 // Use item's zIndex or default to 1
                });
              })}
            </TransitionSeries>
          );
