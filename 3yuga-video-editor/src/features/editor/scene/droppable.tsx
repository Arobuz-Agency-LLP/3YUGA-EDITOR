  ...
  const handleDrop = useCallback((draggedData: DraggedData) => {
    ...
    switch (draggedData.type) {
      ...
      case AcceptedDropTypes.TRANSITION:
        console.warn("Transitions must be applied between clips using the Transitions control panel");
        break;
    }
  }, []);
...
export const DroppableArea: React.FC<DroppableAreaProps> = ({
  ...
  return (
    <div
      ...
      style={style}
      role="region"
      aria-label="Droppable area for images, videos, audio, and transitions"
    >
      {children}
    </div>
  );
